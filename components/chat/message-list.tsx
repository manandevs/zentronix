...
export function MessageList({ messages, isStreaming, error, onRetry, isLoaded }: MessageListProps) {
  ...
  const showTypingIndicator =
    ...
    (messages.length === 0 ||
      ...
      (lastMessage?.sender === "bot" && lastMessage?.text === ""))
    ...
    >
      ...
      {messages
        .filter((message) => {
          ...
          if (isStreaming && message.sender === "bot" && message === lastMessage && message.text === "") {
            return false
          }
          return true
        })
        .map((message, index) => (
          <MessageBubble
            key={index}
            message={message}
            isStreaming={isStreaming && message.sender === "bot" && message === lastMessage}
          />
        ))}
