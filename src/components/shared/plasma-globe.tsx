...
void main(){
  ...
  float alpha = smoothstep(0.0, 0.1, length(col));
...
export default function PlasmaGlobe({
  ...
  useEffect(() => {
    ...
    const onMouse = (e: MouseEvent) => {
      mouseRef.current.x += (e.clientX - mouseRef.current.x) * 0.025;
      mouseRef.current.y += (e.clientY - mouseRef.current.y) * 0.025;
    };
  ...
  return (
    <div
      ...
      style={{ top, left, right, bottom, width, height, transform: `scale(${scale})` }}
    />
  );
}
